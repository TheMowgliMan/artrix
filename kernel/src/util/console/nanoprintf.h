...
#define CYAN COL_ "36m"
#define GREEN COL_ "32m"
...
#define error(fmt, ...) \
	kprintf("[" RED " FAIL " COLOR_RESET "] Error: " fmt __VA_OPT__(,) PASS_ARGS(__VA_ARGS__))
...
#define warning(fmt, ...) \
	kprintf("[" YELLOW " WARN " COLOR_RESET "] Warning: " fmt __VA_OPT__(,) PASS_ARGS(__VA_ARGS__))
#define log(fmt, ...) \
	kprintf("[ NOTE ] " fmt __VA_OPT__(,) PASS_ARGS(__VA_ARGS__))
#define log_proc(fmt, ...) \
	kprintf("[" CYAN " PROC " COLOR_RESET "] Procedure: " fmt __VA_OPT__(,) PASS_ARGS(__VA_ARGS__))
#define log_success(fmt, ...) \
	kprintf("[" GREEN " DONE " COLOR_RESET "] " fmt __VA_OPT__(,) PASS_ARGS(__VA_ARGS__))
